...
export default new Vue({
  methods: {
    ...
    async login(data) {
      sessionStorage.setItem('sessionkey', this.sessionkey)
      this.$http_ndp.defaults.headers.common['token'] = data.user.token
    ...
    getWebURL() {
      ...
      return 'http://121.171.88.159'
      // return 'http://118.128.43.7:30443'
      // return 'https://kodis.or.kr:50443'
    },
    getTaskTitle(task_title, limit) {
      if (task_title != undefined && task_title.length > limit) {
        return task_title.slice(0, limit) + "..."
      }
      return task_title
    },
    getHistContents(hist_contents) {
      if (hist_contents != undefined && hist_contents.length > 30) {
        return hist_contents.slice(0, 30) + "..."
      }
  ...
  data() {
    return {
      ...
      ndp_task_type: [
        ...
        { name: '구축', code: 'GT01' },
        { name: '품질', code: 'GT02' },
        { name: '구축+API', code: 'GT03' },
        { name: '구축+품질', code: 'GT04' },
        { name: '품질+API', code: 'GT05' },
        { name: '구축+품질+API', code: 'GT06' },
      ]
    }
  }
})
